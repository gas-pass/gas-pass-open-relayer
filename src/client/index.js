...
/**
 * GAS PASS program account
 */
let gaspassAccount: Account;
 ...
 * Program Path
 */
const pathToProgram = 'dist/program/gaspass.so';
...
/**
 * Load the GAS PASS program if not already loaded
 */
...
export async function topup(): Promise<void> {
    ...
    const keys = [
        { pubkey: gaspassAccount.publicKey, isSigner: false, isWritable: true },
...
export async function printState(): Promise<void> {
    let data = await connection.getAccountInfo(gaspassAccount.publicKey);
